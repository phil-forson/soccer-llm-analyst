...
def _rank_by_similarity_and_heuristics(
    query: str,
    results: List[dict],
    ...
    """
    Rank results using both semantic similarity and heuristic scoring.
    
    Combines:
    - Semantic similarity (how well content matches query meaning)
    - Heuristic scoring (trusted sources, team mentions, etc.)
    """
    if not results:
        return []
    ...
    for sim_score, r in similarity_scored:
        title = (r.get("title") or "").lower()
        snippet = (r.get("snippet") or "").lower()
        url = (r.get("url") or "").lower()
        text = f"{title} {snippet}"
        
        # Start with similarity score (0-1 range)
        score = sim_score
        
        # Boost for team mentions
        has_home = bool(home and home in text)
        has_away = bool(away and away in text)
        
        if has_home and has_away:
            score += 0.3  # Both teams mentioned
        elif has_home or has_away:
            score += 0.15  # One team mentioned
        
        # Boost for match-like patterns
        if home and away:
            vs_pattern = rf"{home}.*(vs|v|v\.)\s*{away}|{away}.*(vs|v|v\.)\s*{home}"
            score_pattern = rf"\d+\s*[-–]\s*\d+"
        ...
        generic_words = ["overview", "fixtures", "schedule", "table", "tables", "squad"]
        if any(w in url or w in title for w in generic_words):
    ...
    print(f"[Ranking] Final ranking (similarity + heuristics):")
    for i, (score, r) in enumerate(final_scored[:5]):
        print(f"  {i+1}. Score: {score:.3f} | {r.get('title', 'No title')[:60]}")
    
    return [r for _, r in final_scored]
...
def search_with_rag(
    ...
    # Step 3: Rank by semantic similarity + heuristics
    ranked_results = _rank_by_similarity_and_heuristics(
